...
Bump the pin deliberately when you need newer packages; don't track `:latest` silently. Breaking changes to this image will be announced via a `:vN` tag bump (`:v1` → `:v2`). See [Finding the digest](#finding-the-digest) at the end of this README for how to look up `<digest>`.
...
```Dockerfile
# Pin to a digest for reproducibility (see "Finding the digest" at the end).
